...
public class Result<T> implements Serializable {
	private static final long serialVersionUID = 2554344984284696007L;
	...
	public static <T> Result<T> newFailure(int code, String message){
		Result<T> result = new Result<T>();
		result.setCode(code!=200 ? code : -1);
	...
	public static <T> Result<T> newFailure(Result<?> failure){
		Result<T> result = new Result<T>();
		result.setCode(failure.getCode()!=200 ? failure.getCode() : -1);
